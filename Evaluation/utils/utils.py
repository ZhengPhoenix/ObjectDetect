...
def load_data(DATA_DIR, GT_DIR, size=(300,300), debug=False, limit=-1, remove_background=0):
    ...
    for a in file_names:
        ...
        # cv2.circle(img, (gt_list[counter][0], gt_list[counter][1]), 2, (0,255,0),-1)
        # cv2.imshow('img',img)
        # cv2.waitKey(0)
        counter+=1
...
def add_comma(string):
	def add_punc(matched):
		prefix = matched.group('prefix')
		suffix = matched.group('suffix')
		poc = matched.group('addpos')
		return prefix + ',' + suffix
	a = re.compile(r'(?P<prefix>[\d.\-\+e]+)(?P<addpos>[\s\S]+?)(?P<suffix>[\d.\-\+e]+)')
	b = re.compile(r'(]\n)')
	new_string = a.sub(add_punc, string)
	new_string = b.sub('],', new_string)
	return new_string
...
def load_data_4(DATA_DIR, GT_DIR, size=(300,300), debug=False, limit=-1, remove_background = 0):
    ...
    return image_list, gt_list.astype('int32'), file_names
