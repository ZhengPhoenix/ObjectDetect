...
def random_crop(img, gt):
    ...
    # sum_array = myGt.sum(axis=1) # origin
    # tl_index = np.argmin(sum_array) # origin

    sum_array = myGt.sum(axis=1)
    tl_index = np.argmin(sum_array)
    
    min_indices = np.where(sum_array == np.min(sum_array))[0]
    if min_indices.shape[0]>1:
      tl_index = min_indices[np.argmin(myGt[min_indices][:,0])]
...
def __get_cords(cord, min_start, max_end, size = 299 , buf = 5, random_scale=True):
    ...
    while(x_start+size> max_end):
        ...
        if(iter==1000):
            x_start= min_start
            break
    return ( max( x_start, max(min_start, 0) ), min(int(x_start+size), max_end) )
